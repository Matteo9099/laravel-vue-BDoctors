...
@endphp--}}
    ...
    <div class="px-4 pt-5 my-5 text-center border-bottom">
        ...
        <div class="col-lg-6 mx-auto">
            <p class="lead mb-4">Entra a far parte di Bprofessionals.it è Gratis! </p>
        <div class="d-grid gap-2 d-sm-flex justify-content-sm-center mb-5">
            <a href="{{route('admin.professionals.create')}}" type="button" class="btn btn-primary text-white btn-lg px-4 me-sm-3">Completa il tuo profilo</a>
        ...
        <div class="container px-5">
            <img src="{{asset('img/professional-reg.jpg')}}" class="img-fluid border rounded-3 shadow-lg mb-4" alt="docotr_register" width="700" height="500" loading="lazy">
        </div>
        </div>
    </div>
                ...
                <div class="collapse navbar-collapse mt-3" id="navbarSupportedContent">
                    <ul class="navbar-nav m-auto m-2 mb-lg-0">
                        <li class="nav-item m-2" id="nav-hov">
                            <a class="nav-link text-white d-inline" href="{{route('admin.professionals.edit', $professional->slug)}}">Modifica Profilo</a>
                        </li>
                        <li class="nav-item m-2">
                            <a class="nav-link text-white d-inline" href="{{route('admin.leads', $professional->slug)}}">Messaggi ricevuti</a>
                        </li>
                        <li class="nav-item m-2 dropdown">
                            <a class="nav-link text-white d-inline" href="{{route('admin.reviews', $professional->slug)}}">Recensioni ricevute</a>
                        </li>
                        <li class="nav-item m-2 dropdown">
                            <a class="nav-link text-white d-inline" href="{{route('admin.charts', $professional->slug)}}">Statistiche</a>
                        </li>
                        <li class="nav-item m-2 dropdown">
                            <a class="nav-link text-white d-inline" href="{{route('admin.subscription.index', $professional->slug)}}">Aggiungi Sponsor</a>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="row gy-3">
                <div class="col-12 col-sm-6">
                    @if (!$professional->photo)
                        <h4 class="fs-3 text-capitalize fw-bold">{{$professional->user->name}} {{$professional->user->surname}}</h4>
                        <img src=" {{ asset('img/not_found.jpg') }} " alt="not_found_photo" class="img-fluid py-2 rounded-pill" height="200px" width="200px">
                    @else
                        <h4 class="fs-3 text-capitalize fw-bold">{{$professional->user->name}} {{$professional->user->surname}}</h4>
                        <img src=" {{ asset('storage/' . $professional->photo) }} " alt="{{ $professional->id }}_photo" class="img-fluid py-2 rounded-pill" height="200px" width="200px">
                    @endif
                    ...
                    <ul class="nav nav-pills flex-column mb-auto">
                        <li>
                            <p><i class="fa-solid fa-location-pin"></i>
                                @if(!$professional->medical_address)
                                    <span title="studio non inserito/presente">Studio non presente</span>
                                @else
                                    <span title="indirizzo studio medico"> {{$professional->medical_address}}</span>
                                @endif
                            </p>
                        </li>
                        <li>
                            <p title="email"><i class="fa-solid fa-envelope"></i> {{$professional->user->email}}</p>
                        </li>
                        <li>
                            <p><i class="fa-solid fa-phone"></i>
                                ...
                                @else
                                   <span title="numero telefono">{{$professional->phone}}</span>
                                @endif
                            </p>
                        </li>
                        <hr>
                        <li>
                            <form action="{{ route('admin.professionals.destroy', $professional->id) }}" method="POST" class="delete-profile d-flex justify-content-center" data-name="{{$professional->user->surname}}">
                                @csrf
                                @method('DELETE')
